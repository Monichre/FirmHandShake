...
class TiltFx {
  ...
  render () {
    for (let i = 0; i <= this.movingTotal - 1; ++i) {
      ...
      this.DOM.moving[i].style.transform = `translateX(${
        this.translations[i].x
      }px) translateY(${this.translations[i].y}px)`
    }
...
class Slide {
  constructor (el, slideshow) {
    this.DOM = { el: el }
    this.figures = []
    ;[...this.DOM.el.querySelectorAll('.slide__figure')].forEach(figure =>
    ...
    this.DOM.backFromContentCtrl.addEventListener(
      'click',
      this.onClickBackFromContentFn
    )
    ...
    this.DOM.innerTitle = [
      ...this.DOM.title.querySelectorAll('.slide__title-inner')
    ]
    ...
    this.innerTitleMainLetters = [
      ...this.DOM.innerTitle[this.innerTitleTotal - 1].querySelectorAll('span')
    ]
    // total letters.
    ...
    this.DOM.showContentCtrl.addEventListener(
      'click',
      this.onClickShowContentFn
    )
  }
  setCurrent () {
...
class Slideshow {
  ...
  toggleContent (action) {
    ...
    times.switchtime =
      action === 'show'
        ? Number(
          this.animationSettings.staggerFactor *
    ...
    times.slideFigures =
      action === 'show'
        ...
        : pos =>
          Number(
            times.switchtime + pos * this.animationSettings.staggerFactor
          )
      ...
      this.tl
        .to(
          ...
          this.animationSettings.duration,
          {
            ease: this.animationSettings.ease,
            y:
              action === 'show' ? (this.dir === 'up' ? '-101%' : '101%') : '0%'
          },
  ...
  toggleSlides (currentSlide, upcomingSlide) {
    ...
    this.tl.addCallback(
      ...
      this.animationSettings.duration +
        this.animationSettings.staggerFactor * (this.slidesTotal - 1)
    )
    ...
    this.tl.addCallback(
      ...
      this.animationSettings.staggerFactor * (currentSlide.figuresTotal - 1)
    )
        ...
        .to(
          ...
          'begin+=' +
            Number(
              this.animationSettings.staggerFactor *
                (currentSlide.figuresTotal - 1) +
                pos * this.animationSettings.staggerFactor
            )
        )
        .to(
          figure.DOM.slideEl,
...
class Navigation {
  ...
  updateCounter () {
    this.DOM.counterCurrent.innerHTML = this.slideshow.current + 1
  }
  hideNavigationCtrls () {
...
export { CursorFx, Slideshow, Navigation }
